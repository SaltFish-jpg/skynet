...
int
skynet_send(struct skynet_context *context, uint32_t source, uint32_t destination, int type, int session, void *data,
            size_t sz) {
...
int
skynet_sendname(struct skynet_context *context, uint32_t source, const char *addr, int type, int session, void *data,
                size_t sz) {
    if (source == 0) {
        source = context->handle;
    }
...
skynet_initthread(int m) {
    // pthread_setspecific()函数将线程特定值与通过先前调用pthread_key_create()获得的键相关联。
    // 不同的线程可能会将不同的值绑定到同一个键。这些值通常是指向已保留供调用线程使用的动态分配内存块的指针。
    // pthread_getspecific()函数代表调用线程返回当前绑定到指定键的值 。
    // 使用不是从pthread_key_create()获取的键值或在使用pthread_key_delete()删除键之后调用pthread_setspecific()或pthread_getspecific()的效果是未定义的。

    // 函数 pthread_getspecific()返回与给定key 关联的线程特定数据值 。如果没有与key关联的线程特定数据值 ，则返回值 NULL。
    // 如果成功， pthread_setspecific()函数将返回零。否则，将返回错误号以指示错误。
